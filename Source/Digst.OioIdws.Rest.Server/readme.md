...
_**Note**: OWIN middlewares typically make no assumptions on how you store your configuration. It's advised that you don't hardcode configuration settings, but store them in .NET configuration, database, or whatever suits you.
...
In many setups TLS client authentication can be offloaded to the reverse proxy server. In this case, the reverse proxy will perform the TLS handshake and validate the client certificate.The TLS session is terminated at the reverse proxy, and as a consequence, the authorization server cannot rely on it for the client authentication.

This is solved by configuring a preferred strategy for certificate retrieval.
The CertificateStrategyType enumeration defines the supported strategies for retrieving client certificates in the Authorization Server component of the OIOIDWS REST server. It is used in conjunction with the CertificateStrategySection configuration section. Enumeration has the following values:
**Enum Values**
* Connection: retrieves the client certificate directly from the TLS connection. This strategy is chosen by default if no other strategy is configured.
* HttpHeader

Retrieves the client certificate directly from the Client-Cert HTTP header field (https://www.rfc-editor.org/rfc/rfc9440.html).

  ```xml<?xml version="1.0" encoding="utf-8"?>
<configuration>
  <!-- Register the custom section -->
  <configSections>
    <section name="certificateStrategy"
             type="Digst.OioIdws.Rest.Server.AuthorizationServer.CertificateRetrieval.CertificateStrategySection,
                   Digst.OioIdws.Rest.Server" />
  </configSections>

  <!-- Example 1: Use TLS connection to retrieve client certificate -->
  <certificateStrategy type="Connection" />

  <!-- Example 2: Use HTTP header to retrieve client certificate -->
  <!--
  <certificateStrategy type="HttpHeader" value="X-Client-Cert" />
  -->
</configuration>
```
Certificate strategy element contains an optional value attribute to be able to specify other than Client-Cert header (f.x. X-Client-Cert).
